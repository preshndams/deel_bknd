...
const validate = (
  ...
  if (check.error) throw new Error(check.error.details[0].message);

  return check.value;
};

export const inputValidator = (constraint, isMiddleware = true) => {
  if (!constraint) throw new Error("Kindly supply validation schema");
...
export const checkPercent = (amount) => {
    return amount * 0.25;
  };